...
import { imageSizeLimit, maxImageAmount } from '@/lib/validators/image';
...
>(
  (
    ...
    const { accept, maxFiles, maxSize, multiple } = dropzoneOptions;
    ...
    const removeFileFromSet = useCallback(
      (i: number) => {
        if (!value) return;
        const filesArray = fileListToArray(value);
        const newFiles = filesArray.filter((_, index) => index !== i);
        onValueChange(newFiles.length > 0 ? arrayToFileList(newFiles) : null);
      },
      [value, onValueChange],
    );
    ...
    const onDrop = useCallback(
      (acceptedFiles: File[], rejectedFiles: FileRejection[]) => {
        ...
        const currentFiles = fileListToArray(value);
        const newValues: File[] = reSelectAll ? [] : [...currentFiles];
        ...
        onValueChange(newValues.length > 0 ? arrayToFileList(newValues) : null);
